import React, { useEffect, useState } from "react"; //设备借用记录
import {
  AppBar,
  Button,
  Toolbar,
...
export default function DeviceApplyHistory(props) {
  ...
  useEffect(() => {
    if (deviceDetail?.id) {
      fetch(`${MNG_DEVICE_USAGE_URL}?deviceId=${deviceDetail?.id}`, {method:'get'})
        .then((res) => res.json())
      ...
      <AppBar className={classes.appBar}>
        <Toolbar>
          <Typography variant="h6" className={classes.title}>
            {`[${deviceDetail.name+deviceDetail.model}] 借用记录`}
          </Typography>
          <IconButton
            edge="start"
            color="inherit"
